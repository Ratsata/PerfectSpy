...
class Api extends CI_Controller
{
    ...
    public function screen_update()
    {
        ...
        if ($new_img) {
            ...
            if ($saved) {
                ...
                $uploaded = $this->transfer_image_to_screen(FCPATH . TX_LED_DATA_PATH . '/current/192.168.1.128/');
    ...
    private function transfer_image_to_screen($img_path)
    {
        ...
        //$result = $this->ftp->mkdir('ftp/192.168.1.128/', 'auto', 0777);
        //$result = $this->ftp->mkdir('ftp/192.168.1.128/', 0777);
        //$result = $this->ftp->mkdir('ftp/192.168.1.128/192.168.1.128', 0777);
        //echo $img_path;
        $result = $this->ftp->mirror('/var/www/html/tectronix/perfectspy/assets/img/led/current/192.168.1.128/','/192.168.1.128/');
            //if ($result){
                //if ($this->ftp->mirror($img_path, TX_UBICACION_PROYECTO, 'auto', 0664)){
            //}
            //$result = $this->ftp->mirror($img_path, TX_UBICACION_PROYECTO, 'auto', 0664);
        $this->ftp->close();
